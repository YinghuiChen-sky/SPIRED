...
def main(fasta_file, model_dir, saved_folder, device):
    ...
    model.load_state_dict(torch.load(f"{model_dir}/SPIRED.pth", map_location="cpu"), strict=False)
    model.eval()

    # load ESM-2 650M model
    esm_650M_path = f"{model_dir}/esm2_t33_650M_UR50D.pt"
    esm2_650M, _ = esm.pretrained.load_model_and_alphabet(esm_650M_path)
