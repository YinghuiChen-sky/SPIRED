...
help() {
    echo -e "Usage:\n"
    echo -e "bash run_spired_fitness.sh [-i INPUT] [-m MODEL_DIR] [-o FOLDER] [-d DEVICE]\n"
    echo -e "Description:\n"
...
model_dir=$(realpath -e "${SPIRED_DIR}/model")
device=cpu
...
python ${SPIRED_DIR}/run_SPIRED-Fitness.py --fasta_file ${input} --model_dir ${model_dir} --saved_folder ${folder} --device ${device}
