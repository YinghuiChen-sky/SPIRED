...
def main(fasta_file, model_dir, saved_folder, device):
    ...
    # load parameter
    model = SPIRED_Stab(device_list=[device] * 4)
    model.load_state_dict(torch.load(f"{model_dir}/SPIRED-Stab.pth", map_location="cpu"))
    model.eval()
    ...
    with torch.no_grad():
        ...
        # data
        f1d_esm2_3B, f1d_esm2_650M, target_tokens = getStabDataTest(wt_seq, esm2_3B, esm2_650M, esm2_batch_converter, device=device_obj)
        wt_data = {"target_tokens": target_tokens, "esm2-3B": f1d_esm2_3B, "embedding": f1d_esm2_650M}
        f1d_esm2_3B, f1d_esm2_650M, target_tokens = getStabDataTest(mut_seq, esm2_3B, esm2_650M, esm2_batch_converter, device=device_obj)
