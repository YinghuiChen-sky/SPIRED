...
@click.option("--model_dir", default=f"{working_directory}/model", show_default=True, type=str)
@click.option("--saved_folder", required=True, type=str)
@click.option("--device", default="cpu", show_default=True, type=str)
def main(fasta_file, model_dir, saved_folder, device):

    if device.startswith("cuda") and not torch.cuda.is_available():
        raise click.ClickException("CUDA is not available, please use --device cpu.")

    try:
        device_obj = torch.device(device)
    except RuntimeError as exc:
        raise click.ClickException(f"Invalid device: {device}") from exc

    # load parameter
    model = SPIRED_Fitness_Union(device_list=[device] * 4)
    model.load_state_dict(torch.load(f"{model_dir}/SPIRED-Fitness.pth", map_location="cpu"))
    model.eval()

    # load ESM-2 650M model
    esm2_650M_path = f"{model_dir}/esm2_t33_650M_UR50D.pt"
    esm2_650M, _ = esm.pretrained.load_model_and_alphabet(esm2_650M_path)
    esm2_650M = esm2_650M.to(device_obj)
    esm2_650M.eval()

    # load ESM-2 3B model
    esm2_3B_path = f"{model_dir}/esm2_t36_3B_UR50D.pt"
    esm2_3B, esm2_alphabet = esm.pretrained.load_model_and_alphabet(esm2_3B_path)
    esm2_3B = esm2_3B.to(device_obj)
    esm2_3B.eval()
    esm2_batch_converter = esm2_alphabet.get_batch_converter()

    # load 5 ESM-1v models
    esm1v_1 = f"{model_dir}/esm1v_t33_650M_UR90S_1.pt"
    esm1v_2 = f"{model_dir}/esm1v_t33_650M_UR90S_2.pt"
    esm1v_3 = f"{model_dir}/esm1v_t33_650M_UR90S_3.pt"
    esm1v_4 = f"{model_dir}/esm1v_t33_650M_UR90S_4.pt"
    esm1v_5 = f"{model_dir}/esm1v_t33_650M_UR90S_5.pt"
    ...
    esm1v_1 = esm1v_1.to(device_obj)
    esm1v_2 = esm1v_2.to(device_obj)
    esm1v_3 = esm1v_3.to(device_obj)
    esm1v_4 = esm1v_4.to(device_obj)
    esm1v_5 = esm1v_5.to(device_obj)
    esm1v_1.eval()
    esm1v_2.eval()
    esm1v_3.eval()
    esm1v_4.eval()
    ...
    with torch.no_grad():
        for protein_name in fasta_dict.keys():
            ...
            f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, target_tokens = getDataTest(seq, esm2_3B, esm2_650M, esm1v_1, esm1v_2, esm1v_3, esm1v_4, esm1v_5, esm1v_batch_converter, esm1v_alphabet, esm2_batch_converter, device=device_obj)
